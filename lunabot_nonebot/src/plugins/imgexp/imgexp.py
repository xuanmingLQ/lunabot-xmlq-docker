...
async def search_saucenao(
    ...
    try:
        saucenao_apikey = config.get('saucenao_apikey')
        if not saucenao_apikey:
            raise Exception("没有提供 saucenao_apikey")
        if limit == 0: 
        ...
        async with Network(timeout=20, proxies=None) as client:
            saucenao = SauceNAO(client=client, api_key=saucenao_apikey, numres=limit)
...
async def search_googlelens(
    ...
    try:
        ...
        if not serp_apikey:
            raise Exception("没有提供 serp_apikey")
        if limit == 0:
